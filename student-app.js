    ...
    try {
        ...
        if (profileSnap.exists()) {
            ...
            if (coachId && studentDocId) {
                ...
                // DÜZELTME: Bildirimleri profil yüklendikten sonra başlat
                initNotifications(); 
                listenHeaderMessages(coachId);
            } else {
...
// ... (btnMatch Profile Eşleştirme aynı) ...
...
// =================================================================
// 4. BİLDİRİMLER VE HEADER (DÜZELTİLDİ)
// =================================================================
...
// DÜZELTME: Öğrenciye özel bildirimleri yükle
function initNotifications() {
    ...
    let notifications = { appointments: [], homeworks: [] };

    const renderNotifications = () => {
        const all = [...notifications.appointments, ...notifications.homeworks];
    ...
    listeners.notifications = onSnapshot(query(collection(db, "artifacts", appId, "users", coachId, "ajandam"), where("studentId", "==", studentDocId), where("tarih", ">=", today), orderBy("tarih", "asc"), limit(3)), (snap) => {
        ...
        snap.forEach(d => {
            ...
            notifications.appointments.push({
                ...
                desc: `${formatDateTR(data.tarih)} ${data.baslangic} - ${data.baslik}`,
                badgeText: 'Seans',
    ...
    // 2. Yapılacak (Aktif) Ödevler
    // Not: collectionGroup yerine direkt öğrenci altındaki ödevleri dinliyoruz
    ...
    // Yeni bir listener ekle (activeListeners listesine)
    listeners.activeGoals = onSnapshot(qOdev, (snap) => {
        ...
        snap.forEach(d => {
            ...
            notifications.homeworks.push({
                title: 'Ödev',
                desc: `${data.title} (${formatDateTR(data.bitisTarihi)})`,
                badgeText: 'Ödev',
...
function listenHeaderMessages(coachUid) {
    // Mesaj sayısını dinle
    const q = query(
        collection(db, "artifacts", appId, "users", coachUid, "ogrencilerim", studentDocId, "mesajlar"), 
        where('gonderen', '==', 'koc'), 
        where('okundu', '==', false)
    );

    listeners.unreadMsg = onSnapshot(q, (snapshot) => {
        const b = document.getElementById('headerUnreadMsgCount');
        if(snapshot.size > 0) { 
            b.textContent = snapshot.size > 99 ? '99+' : snapshot.size; 
...
async function loadUpcomingAppointments(db, uid, appId, sid) {
    ...
    if (snap.empty) {
        container.innerHTML = '<p class="text-center text-gray-400 text-xs py-4">Planlanmış seans yok.</p>';
    } else {
...
async function loadAllUpcomingAppointments(container, todayStr) {
    ...
    if (snap.empty) { container.innerHTML = '<p class="text-center text-xs text-gray-400 py-4">Planlanmış seans yok.</p>'; } 
        ...
        container.innerHTML = snap.docs.map(doc => {
            const a = doc.data(); const isToday = a.tarih === todayStr;
            return `<div class="bg-white p-3 rounded-xl border-l-4 ${isToday ? 'border-green-500 shadow-md' : 'border-indigo-500 shadow-sm'} mb-2 flex justify-between items-center"><div><p class="font-bold text-sm text-gray-800">${a.baslik || 'Seans'}</p><p class="text-xs text-gray-500 flex items-center gap-1"><i class="fa-regular fa-calendar"></i> ${formatDateTR(a.tarih)} <i class="fa-regular fa-clock ml-1"></i> ${a.baslangic}</p>${a.not ? `<p class="text-[10px] text-gray-400 mt-1 italic">"${a.not}"</p>` : ''}</div>${isToday ? '<span class="text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded font-bold">BUGÜN</span>' : ''}</div>`;
        }).join('');
    }
}
