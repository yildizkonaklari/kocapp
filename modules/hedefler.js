...
export async function renderHedeflerSayfasi(db, currentUserId, appId) {
    ...
    // 2. Yeni Hedef Ekleme Butonu (Geri Tuşu Destekli Modal)
    ...
    const renderList = (filter = "") => {
        ...
        filtered.forEach(s => {
            ...
            item.onclick = () => {
                ...
                dropdown.classList.add('hidden'); // Listeyi Kapat
...
function openAddModal() {
    ...
    // Modal içindeki Kapat/İptal butonlarını ayarla (Geri tuşu gibi davransın)
    const modal = document.getElementById('addHedefModal');
    const closeBtn = modal.querySelector('.fa-xmark').closest('button');
