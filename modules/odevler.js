...
export async function renderOdevlerSayfasi(db, currentUserId, appId) {
    ...
    area.innerHTML = `
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100 relative z-20">
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button id="btnApproveAllOdev" class="hidden flex-1 md:flex-none bg-green-100 text-green-700 px-4 py-2.5 rounded-xl hover:bg-green-200 transition-colors text-sm font-bold border border-green-200 items-center justify-center">
                    <i class="fa-solid fa-check-double mr-2"></i> Tümünü Onayla
                </button>

                <button id="btnAddNewOdev" class="hidden flex-1 md:flex-none bg-purple-600 text-white px-5 py-2.5 rounded-xl hover:bg-purple-700 shadow-lg shadow-purple-200 flex items-center justify-center transition-transform active:scale-95 text-sm font-medium">
    ...
    // Tümünü Onayla Listener
    document.getElementById('btnApproveAllOdev').addEventListener('click', () => {
        approveAllOdevs();
    });
}
    ...
    const renderList = (filter = "") => {
        ...
        filtered.forEach(s => {
            ...
            item.onclick = () => {
                ...
                document.getElementById('btnApproveAllOdev').classList.remove('hidden');
...
function openAddOdevModal() {
    ...
    // Hata Kontrolü: Input var mı kontrol et
    const hiddenInput = document.getElementById('currentStudentIdForOdev');
    if (hiddenInput) hiddenInput.value = sid;
    else { alert("Sistem Hatası: 'currentStudentIdForOdev' alanı bulunamadı. Lütfen coach-dashboard.html dosyasını güncelleyin."); return; }
...
export async function saveGlobalOdev() {
    ...
    try {
        ...
        // ÖNEMLİ: Sadece modalı kapat (Geçmişten sil). Sayfayı yenileme.
        window.history.back();
...
async function approveAllOdevs() {
    const sid = document.getElementById('filterOdevStudentId').value;
    if (!sid) return;

    if (!confirm("Bu öğrenciye ait 'Devam Eden' ve 'Onay Bekleyen' tüm ödevler TAMAMLANDI olarak işaretlenecek. Emin misiniz?")) return;
    ...
    try {
        const batch = writeBatch(db);
        const ref = collection(db, "artifacts", appId, "users", uid, "ogrencilerim", sid, "odevler");
        
        // Hem 'devam' edenleri hem de 'onayDurumu: bekliyor' olanları çekip güncellemek en garantisidir.
        // Ancak Firestore'da OR sorgusu karmaşık olabilir. Basitçe tüm ödevleri çekip JS tarafında filtreleyelim (Client-side filtering).
        // Ödev sayısı çok aşırı olmayacağı için bu yöntem güvenlidir.
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // Güncellenmesi gerekenler: Durumu 'devam' olanlar VEYA (Durumu 'tamamlandi' ama onayı 'bekliyor' olanlar)
            if (data.durum === 'devam' || (data.durum === 'tamamlandi' && data.onayDurumu === 'bekliyor')) {
        ...
        if (updateCount > 0) {
            await batch.commit();
            // alert(`${updateCount} adet ödev onaylandı.`); // Opsiyonel bildirim
        } else {
            alert("Onaylanacak yeni ödev bulunamadı.");
        }

    } catch (e) {
        console.error(e);
        alert("Toplu onay işleminde hata oluştu.");
    } finally {
        btn.disabled = false;
