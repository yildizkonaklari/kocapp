...
// Modül Importları
import { cleanUpListeners, populateStudentSelect, renderDersSecimi, renderPlaceholderSayfasi, formatDateTR, renderStudentOptions } from './modules/helpers.js';
...
// 1. Okunmamış Mesaj Sayısını Dinle
function listenUnreadMessages(uid) {
    const badge = document.getElementById('headerUnreadMsgCount'); // Header'daki sayı (eğer varsa)
    // Eğer HTML'de bu ID yoksa hata vermemesi için kontrol
    if(!badge) return;

    // Koç panelinde mesajlar 'gonderen: ogrenci' olanlardır
    const q = query(
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // Buton üzerinde sayı göster (HTML'de bu ID'ye sahip bir span olmalı)
        // Genelde icon içinde olur. Eğer yoksa eklemeyi deneyebilirsiniz.
        // Mevcut iconun üzerine badge eklemek için:
        ...
        if(btnMsg) {
             ...
             if(!badgeSpan) {
                 ...
                 badgeSpan.className = 'msg-badge absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white hidden';
...
// 2. Yönlendirmeli Bildirim Sistemi
function initNotifications(uid) {
    ...
    let notifications = { 
        ...
        pendingQuestions: [], 
        pendingExams: [],
        pendingHomeworks: [] // YENİ: Ödevler
    };

    const renderNotifications = () => {
        const all = [
            ...notifications.appointments,
            ...notifications.pendingHomeworks, // Önce işler
        ...
        if (all.length > 0) {
            ...
            let html = '';
            all.forEach(item => {
                // Yönlendirme Linki (Sidebar butonuna tıklama simülasyonu)
                html += `
    ...
    onSnapshot(query(collection(db, "artifacts", appId, "users", uid, "ajandam"), where("tarih", ">=", today), orderBy("tarih", "asc"), limit(5)), (snap) => {
        ...
        snap.forEach(d => {
            const data = d.data();
            // Sadece bugün ve yarın olanları göster (veya tüm gelecekleri)
    ...
    // 4. YENİ: Onay Bekleyen Ödevler (Tamamlanmış ama Onaylanmamış)
...
// Kapatma Butonları
document.querySelectorAll('.close-modal-btn, #closeModalButton, #closeEditModalButton, #closeDenemeModalButton, #closeSoruModalButton, #closeHedefModalButton, #closeOdevModalButton, #closeRandevuModalButton, #closeEditRandevuModalButton, #closeTahsilatModalButton, #closeBorcModalButton, #closeProfileModalButton, #cancelModalButton, #cancelEditModalButton, #cancelDenemeModalButton, #cancelSoruModalButton, #cancelHedefModalButton, #cancelOdevModalButton, #cancelRandevuModalButton, #cancelEditRandevuModalButton, #cancelTahsilatModalButton, #cancelBorcModalButton').forEach(btn => {
