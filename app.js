...
let soruTakibiUnsubscribe = null; // Aktif Firestore dinleyicisini tutar
let soruTakibiZaman = 'haftalik'; // 'haftalik' or 'aylik'
let soruTakibiOffset = 0; // 0 = mevcut hafta/ay, -1 = önceki, 1 = sonraki
...
async function main() {
    ...
    // Firebase loglamasını aç (hata ayıklama için)
    setLogLevel('debug');

    // Auth (Giriş) durumunu dinle
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            currentUserId = null;
            console.log("Kullanıcı çıkış yaptı veya bulunamadı. Yeniden deneniyor...");
            // (signInAnonymously veya signInWithCustomToken tekrar tetiklenebilir veya giriş ekranı gösterilebilir)
        }
    });

    // Canvas ortamı için otomatik giriş yap
    try {
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
            // Canvas'ın verdiği özel token ile giriş yap
            await signInWithCustomToken(auth, __initial_auth_token);
        } else {
            // Eğer token yoksa (lokal test vb. için) anonim giriş yap
            await signInAnonymously(auth);
        }
    } catch (error) {
        console.error("Firebase giriş hatası:", error);
        loadingSpinner.innerHTML = `<p class='text-red-500'>Giriş Başarısız: ${error.message}</p>`;
    }
}

// 5. Arayüz Güncelleme Fonksiyonları

// Giriş yapan koçun bilgilerini sol menüye yazar
function updateUIForLoggedInUser(user) {
    if (user) {
        // Anonim kullanıcıysa "Koç" yaz, değilse e-postasını
    ...
    logoutButton.addEventListener('click', () => {
        signOut(auth).then(() => {
            ...
            window.location.reload(); // Sayfayı yeniden yükleyerek en temiz çıkışı yap
        });
    });
        ...
        link.addEventListener('click', (e) => {
            ...
            switch(pageId) {
                case 'ogrencilerim':
                    renderOgrenciSayfasi();
                    break;
                // DİĞER SAYFALAR BURAYA EKLENECEK
                default:
...
// "Öğrencilerim" sayfasının ana arayüzünü çizer
function renderOgrenciSayfasi() {
    if (!currentUserId) return; // Giriş yapmadan işlem yapma
    ...
    // "Yeni Ekle" butonunu modal'a bağla
    document.getElementById('showAddStudentModalButton').addEventListener('click', () => {
        // Modalı göstermeden önce formu temizle
        document.getElementById('studentName').value = '';
        document.getElementById('studentSurname').value = '';
        document.getElementById('studentClass').value = '12. Sınıf'; // Varsayılan
        modalErrorMessage.classList.add('hidden');
        addStudentModal.style.display = 'block';
    });
    
    // Öğrencileri Firestore'dan yükle
    loadOgrenciler();
}

// Firestore'dan öğrencileri gerçek zamanlı olarak yükler
function loadOgrenciler() {
    const studentListContainer = document.getElementById('studentListContainer');
    if (!studentListContainer) return; // Sayfa değişmişse konteyner bulunamayabilir

    // Sadece bu koça ait öğrencileri yükle
    // GÜVENLİ YOL: /artifacts/{appId}/users/{userId}/ogrencilerim
    const q = query(collection(db, "artifacts", appId, "users", currentUserId, "ogrencilerim"));
    
    // onSnapshot: Veritabanı her değiştiğinde (ekleme, silme, güncelleme) bu fonksiyon çalışır
...
// Alınan öğrenci verisini HTML tablosuna dönüştürür
function renderStudentList(students) {
    ...
    // Listeye eklenen "Sil" butonlarına tıklama olayı ekle
...
async function saveNewStudent() {
    ...
    try {
        ...
        // Veriyi "koclar" -> [koçun_id'si] -> "ogrencilerim" koleksiyonuna ekle
        // GÜVENLİ YOL: /artifacts/{appId}/users/{userId}/ogrencilerim
        const docRef = await addDoc(collection(db, "artifacts", appId, "users", currentUserId, "ogrencilerim"), {
            ad: ad,
            soyad: soyad,
            sinif: sinif,
            olusturmaTarihi: serverTimestamp() // Sunucu zamanını kullan
        });
        
        console.log("Öğrenci eklendi, ID:", docRef.id);
        
        // Başarılı eklemeden sonra modalı kapat
...
// Öğrenci detay sayfasının ana arayüzünü çizer
function renderOgrenciDetaySayfasi(studentId, studentName) {
    // Ana başlığı ve içeriği temizle/ayarla
    ...
    mainContentArea.innerHTML = `
        ...
        <div class="bg-white p-6 rounded-lg shadow-md flex flex-col md:flex-row items-center mb-6 gap-4">
            ...
            <div class="text-center md:text-left">
                ...
                <p class="text-lg text-gray-500" id="studentDetailClass"></p> <!-- Özet sekmesi dolduracak -->
            </div>
    ...
    // "Bilgileri Düzenle" butonunu bağla
    ...
    document.querySelectorAll('.tab-button').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            // Tıklanan sekmeye göre içeriği yükle
            ...
            switch(tabId) {
                case 'ozet': 
                    ...
                    break;
                case 'denemeler': 
                    renderDenemelerTab(studentId, studentName); 
                    break;
                case 'soru-takibi': 
                    ...
                    break;
                case 'hedefler': 
                    renderHedeflerOdevlerTab(studentId, studentName); 
                    break;
                case 'notlar': 
                    renderKoclukNotlariTab(studentId, studentName); 
                    break;
                default: 
                    renderPlaceholderTab(tabId); 
                    break;
            }
        });
    });
    
    // Varsayılan olarak "Özet" sekmesini yükle
    renderOzetTab(studentId);
}
...
async function renderOzetTab(studentId) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            console.log("Öğrenci detayı bulunamadı!");
...
function renderDenemelerTab(studentId, studentName) {
    ...
    document.getElementById('showAddDenemeModalButton').addEventListener('click', () => {
        ...
        renderDenemeNetInputs('TYT'); // Varsayılanı çiz
...
// Sınav türüne göre D/Y/B giriş alanlarını çizer
...
// Firestore'dan denemeleri yükler
function loadDenemeler(studentId) {
    ...
    const q = query(collection(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "denemeler"));
...
// Alınan deneme verisini HTML'e dönüştürür
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sınav Adı</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tür</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Toplam Net</th>
                    <th scope="col" class="relative px-6 py-3"><span class="sr-only">Eylemler</span></th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                ${denemeler.map(deneme => {
                    let toplamNet = 0;
                    if (deneme.toplamNet !== undefined) {
                        toplamNet = deneme.toplamNet;
                    } else {
                        const turkceNet = (deneme.turkceD || 0) - (deneme.turkceY || 0) / 4;
                        const matNet = (deneme.matD || 0) - (deneme.matY || 0) / 4;
                        toplamNet = turkceNet + matNet;
                    }
                    const toplamNetStr = toplamNet.toFixed(2);
                    ...
                    return `
                        <tr id="deneme-row-${deneme.id}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${turClass}">
                                    ${deneme.tur}
                                </span>
                            </td>
...
// Yeni denemeyi Firestore'a kaydeder
...
// Tarih aralığı hesaplama yardımcısı
function getSoruTakibiDateRange(zaman, offset) {
    ...
    if (zaman === 'haftalik') {
        const dayOfWeek = today.getDay() === 0 ? 6 : today.getDay() - 1; // 0=Pzt, 6=Paz
...
// Soru takibi özet panelini günceller
...
// Soru Takibi sekmesinin ana arayüzünü çizer
function renderSoruTakibiTab(studentId, studentName) {
    ...
    tabContentArea.innerHTML = `
        ...
        <div id="soruTakibiSummary" class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm flex justify-center items-center col-span-2 md:col-span-1">
                <div id="summaryDonutChart" class="w-24 h-24">
                    <!-- Donut grafik buraya gelecek -->
                </div>
            </div>
        </div>

        <!-- Soru Verisi Listesi Alanı -->
    ...
    // "Yeni Veri Ekle" butonunu modal'a bağla
    ...
    document.querySelectorAll('.soru-zaman-toggle').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            loadSoruTakibi(studentId); // Veriyi yeniden yükle
        });
    });

    // Tarih navigasyon butonlarını bağla
    ...
    // Bu öğrenciye ait soru verilerini yükle (ilk yükleme)
    loadSoruTakibi(studentId);
}

// Firestore'dan soru verilerini yükler
...
// Alınan soru verisini HTML'e dönüştürür
...
// Yeni soru verisini Firestore'a kaydeder
...
// Ana Hedefler/Ödevler sekmesini çizer
function renderHedeflerOdevlerTab(studentId, studentName) {
    ...
    renderHedeflerSubTab(studentId, studentName); // Varsayılan
}

// Hedefler Alt Sekmesini çizer
function renderHedeflerSubTab(studentId, studentName) {
    ...
    subTabContentArea.innerHTML = `
        ...
        <div id="hedefListContainer">
            <p class="text-gray-500 text-center py-4">Hedefler yükleniyor...</p>
        </div>
    `;
...
function renderOdevlerSubTab(studentId, studentName) {
    ...
    subTabContentArea.innerHTML = `
        ...
        <div id="odevListContainer">
            <p class="text-gray-500 text-center py-4">Ödevler yükleniyor...</p>
        </div>
    `;
...
// Firestore'dan hedefleri yükler
...
function renderHedeflerList(hedefler, studentId) {
    ...
    document.querySelectorAll('.toggle-hedef-button').forEach(btn => btn.addEventListener('click', (e) => {
        ...
        const hedefDocRef = doc(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "hedefler", id);
...
function renderOdevlerList(odevler, studentId) {
    ...
    document.querySelectorAll('.toggle-odev-button').forEach(btn => btn.addEventListener('click', (e) => {
        const id = e.currentTarget.dataset.id;
        const newStatus = e.currentTarget.dataset.status;
        const odevDocRef = doc(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "odevler", id);
...
async function saveNewHedef() {
    ...
    try {
        ...
        await addDoc(collection(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "hedefler"), {
            ...
            aciklama: aciklama,
            durum: "devam", // 'devam' veya 'tamamlandi'
...
async function saveNewOdev() {
    ...
    try {
        ...
        await addDoc(collection(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "odevler"), {
            title: title,
...
// Koçluk notları listesini HTML'e çizer
function renderKoclukNotlariList(notlar, studentId) {
    ...
    document.querySelectorAll('.delete-not-button').forEach(btn => btn.addEventListener('click', async (e) => {
        ...
        if (confirm('Bu koçluk notunu silmek istediğinize emin misiniz?')) {
            const notDocRef = doc(db, "artifacts", appId, "users", currentUserId, "ogrencilerim", studentId, "koclukNotlari", id);
            await deleteDoc(notDocRef);
        }
    }));
}

// Yeni koçluk notunu kaydeder
...
async function saveStudentChanges() {
    ...
    try {
        ...
        // Arayüzü canlı güncelle (Eğer profil sayfasındaysak)
...
// === 9. MODAL KONTROLLERİ ===
// (Tüm modal açma butonları artık ilgili render fonksiyonları içinde)

// Öğrenci Ekleme Modalı (Kapatma/Kaydetme)
...
// Öğrenci Düzenleme Modalı (Kapatma/Kaydetme)
...
// Deneme Modalı (Kapatma/Kaydetme)
...
// Soru Takibi Modalı (Kapatma/Kaydetme)
...
// Koçluk Notu Modalı (Kapatma/Kaydetme)
...
// Dinamik Deneme Türü değişikliğini dinle
